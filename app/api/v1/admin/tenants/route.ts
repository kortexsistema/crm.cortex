...
export async function POST(req: NextRequest) {
  ...
  const { display_name, slug, legal_name, cnpj, plan, owner_email, owner_password } = parsed.data;
  const admin = createAdminClient();
  ...
  // -------------------------------------------------------------------------
  // Orquestração do Usuário Responsável
  // -------------------------------------------------------------------------
  let ownerId: string;
  const { data: authUser, error: authError } = await admin.auth.admin.createUser({
    ...
    email_confirm: true,
    user_metadata: {
      skip_org_provision: "true",
    },
  });

  if (authError) {
    if (authError.status === 422 || authError.message.includes("already exists") || authError.message.includes("User already registered")) {
      const { data: rpcId, error: rpcError } = await admin.rpc("fn_get_user_id_by_email", {
        p_email: owner_email,
      });
      if (rpcError || !rpcId) {
        return fail("internal_error", "Failed to resolve existing owner user", 500, {
          requestId,
          details: rpcError?.message || "User not found via RPC",
        });
      }
      ownerId = rpcId;
    } else {
      return fail("internal_error", "Failed to create owner user", 500, {
        requestId,
  ...
  const { error: bindError } = await admin.from("user_organizations").insert({
    user_id: ownerId,
  ...
  if (bindError) {
    return fail("internal_error", "Failed to bind owner to tenant", 500, {
      requestId,
