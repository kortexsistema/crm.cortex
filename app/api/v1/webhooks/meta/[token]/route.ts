...
export async function POST(req: NextRequest, ctx: RouteCtx): Promise<NextResponse> {
  ...
  for (const e of eventos) {
    ...
    if (e.kind === "template_status") {
      ...
      await admin
        .from("messages")
        .update({ status: isFailed ? "failed" : "sent", updated_at: now })
        .eq("organization_id", session.organizationId)
        .eq("external_id", e.externalId);

      if (isFailed) {
        // Descobrir de quem era a mensagem para o log. (Onda 5: reatividade do follow-up aborta fluxos do lead se a entrega falha).
        ...
        if (msg?.contact_id) {
          await admin.rpc("emit_event", {
            _org_id: session.organizationId,
            _type: "message.failed",
            _payload: { contact_id: msg.contact_id, external_id: e.externalId, reason: "delivery_failed" },
            _idempotency_key: `meta_message_failed:${e.externalId}`,
          });
        }
      }
    }
