...
export function describeNodeConfig(type: NodeType, config: FlowNode["config"]): string {
  switch (type) {
    ...
    case "wait": {
      ...
      if (c.mode === "fixed") return `${Math.round(c.duration_ms / 60_000)} min`;
      if (c.mode === "event_relative") return `Evento ${c.offset_ms >= 0 ? "+" : ""}${Math.round(c.offset_ms / 60_000)} min`;
      return `${Math.round(c.min_ms / 60_000)}–${Math.round(c.max_ms / 60_000)} min (adaptativo)`;
    }
    case "condition": {
      const c = config as ConfigOf<"condition">;
      if ("rules" in c) {
        return `${c.rules.length} regra(s) distintas`;
      }
