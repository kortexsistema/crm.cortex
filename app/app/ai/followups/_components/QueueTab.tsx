...
export function QueueTab({ canWrite }: Props) {
  ...
  const [pendingAdvance, setPendingAdvance] = useState<FollowupQueueRow | null>(null);
  ...
  const advanceEnrollment = useAdvanceFollowupEnrollment();
  ...
  return (
    <div className="flex flex-col gap-4">
                    ...
                    {canWrite && (
                      <TableCell>
                        <div className="flex gap-2">
                          {canCancel && row.source === "enrollment" && (
                            <Button
                              variant="ghost"
                              size="sm"
                              data-testid="forcar-avanco-fila"
                              aria-label="Forçar Avanço"
                              onClick={() => setPendingAdvance(row)}
                            >
                              <Play size={14} aria-hidden className="mr-1 text-info" /> Avançar
                            </Button>
                          )}
                          {canCancel && (
                            <Button
      ...
      <AlertDialog open={pendingAdvance !== null} onOpenChange={(open) => !open && setPendingAdvance(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Forçar Avanço?</AlertDialogTitle>
            <AlertDialogDescription>
              Isso fará com que o contato pule o tempo de espera atual e vá imediatamente para a próxima etapa do fluxo.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Voltar</AlertDialogCancel>
            <AlertDialogAction
              ...
              onClick={() => {
                if (pendingAdvance) advanceEnrollment.mutate(pendingAdvance.id);
