...
function StatusBadge({
  ...
  let effective = status === "active" && !onboardedAt ? "onboarding" : status;
  
  if (effective === "active" && subscriptionStatus) {
    if (subscriptionStatus === "pending_payment") effective = "pending_payment";
    else if (subscriptionStatus === "expiring") effective = "expiring";
    else if (subscriptionStatus === "suspended") effective = "suspended";
  }
  ...
  return (
    <div className="rounded-md border">
      <Table>
        <TableHeader>
          <TableRow>
            {["Slug", "Nome", "CNPJ", "Status", "Users", "Conversas", "Vencimento", "Contato", "Criado em", ""].map(
              (h) => (
              ...
              <TableRow key={row.id}>
                ...
                <TableCell>
                  <StatusBadge 
                    ...
                    subscriptionStatus={row.settings?.subscription_status} 
                  />
                </TableCell>
                ...
                <TableCell className="text-xs text-muted-foreground">
                  {formatDate(row.settings?.billing_due_date ?? null)}
                </TableCell>
                <TableCell className="text-xs text-muted-foreground truncate max-w-[120px]">
                  {row.settings?.billing_contact_phone || "—"}
                </TableCell>
                <TableCell className="text-xs text-muted-foreground">
