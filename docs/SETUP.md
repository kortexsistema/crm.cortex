...
- Fica na **raiz do projeto**: `/seu-caminho/Kortex/.env.local`.
...
2. No dashboard, clique **New project**.
   - **Name:** `kortex-dev` (ou o que quiser).
...
3. Clique **Create API Key** → nome `kortex-dev` → copie a chave.
...
3. **Settings → API Keys → Create Key** → nome `kortex-dev` → copie.
...
3. **API Keys → Create new secret key** → nome `kortex-dev-embeddings` → copie.
...
- Confira logs do container: `docker logs kortex-waha`.
...
Bem-vindo ao Kortex. 🛠️

---

> **Achou um erro neste guia?** Abra uma [issue](https://github.com/kortexsistema/kortex-v1/issues) ou mande um PR — esse doc vive da contribuição da comunidade.
