---
title: Kortex — Catálogo de Regras de Negócio
version: 0.1
status: em revisão
date: 2026-04-28
...
# Kortex — Catálogo de Regras de Negócio
...
- **Tipo**: Soft policy
- **Regra**: GIVEN request entrando pela API; WHEN chega no edge; THEN tenant é resolvido via subdomain (`<tenant>.api.kortex.com`) OU header `X-Tenant-ID` (configurável global). API key NÃO determina tenant — o JWT/Bearer determina.
...
- **Exceção**: Custos administrativos da plataforma (super-admin testando, suporte) são debitados ao tenant `internal_kortex`.
