...
- **Worktree:** `/Users/rafaelmelgaco/Kortex-crm-vivo`
...
Existem outros worktrees vivos (`.claude/worktrees/*`, `Kortex-qa`, `Kortex-main-preview`,
`Kortex-vendaval*`). **Nenhum deles é seu.** Trabalhe só no worktree acima.

---

## 1. A missão

O Kortex existe para que **todo lead que entra vire uma venda feita ou um problema
...
`KORTEX_GOV_INVARIANTS_EDIT=1` com o `+N −0` **medido** no corpo do commit. Se o `−0` não
