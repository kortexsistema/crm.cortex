...
### Padrões roubáveis
1. **Datetime como mensagem user-role separada** para não invalidar prompt cache em jobs recorrentes (`task_scheduler.py:1578-1587`) — system prompt fica byte-idêntico. (O Kortex já faz o equivalente no `followup-turn.ts`: bloco temporal no sufixo.)
