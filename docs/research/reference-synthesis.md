...
**Status:** Adotada integralmente como linha de base arquitetural do Kortex (decisão registrada em memória do projeto).

Esse documento extrai apenas as decisões e padrões da referência que o Kortex herda. Para citações literais, schema SQL completo e edge cases detalhados, consultar a fonte original.
...
## 11. Gaps a desenhar do Kortex (não cobertos pela referência)
...
5. **Super-admin de plataforma** — coluna `is_platform_admin` ou tabela separada; helper RLS retorna TRUE para essa role; UI separada (`admin.kortex.com`)
