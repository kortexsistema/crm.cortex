...
2. Domínio com DNS gerenciado em Cloudflare (ou outro provider) — ex.: `waha.kortex.com.br`.
3. Conta Backblaze B2 com bucket `kortex-waha-backup` (R$0,06/GB/mês ≈ $0.005/GB).
...
# usuário não-root
adduser kortex
usermod -aG sudo kortex
mkdir -p /home/kortex/.ssh
cp ~/.ssh/authorized_keys /home/kortex/.ssh/
chown -R kortex:kortex /home/kortex/.ssh
chmod 700 /home/kortex/.ssh && chmod 600 /home/kortex/.ssh/authorized_keys
...
A partir daqui: `ssh kortex@<IP>` + `sudo` para tudo.
...
```bash
sudo tee /etc/fail2ban/jail.d/kortex.conf > /dev/null <<'EOF'
[sshd]
enabled = true
maxretry = 3
bantime = 1h
...
- `waha.kortex.com.br` → A record → IP do VPS Hostgator
- Proxy = **DNS only** (cinza). Cloudflare Proxy (laranja) interfere em SSE/WebSocket que o WAHA usa.

```bash
sudo certbot --nginx -d waha.kortex.com.br \
  --non-interactive --agree-tos -m ops@kortex.com.br
```
...
```bash
sudo mkdir -p /opt/kortex-waha
sudo chown kortex:kortex /opt/kortex-waha
cd /opt/kortex-waha
```

Copiar (via `scp` ou `git clone`) o `docker-compose.yml` do repo (raiz do Kortex). Ajustes obrigatórios pra prod:
...
`/opt/kortex-waha/.env` (chmod 600):

```bash
WAHA_API_KEY=<plaintext gerado novo, 64 chars hex>
WAHA_WEBHOOK_BASE_URL=https://app.kortex.com.br
WAHA_HMAC_SECRET=<32 bytes random distinto da api key>
```

```bash
chmod 600 /opt/kortex-waha/.env
```
...
echo "0 4 * * * kortex /usr/local/bin/refresh-vercel-cidrs.sh" | sudo tee /etc/cron.d/vercel-cidrs
```
...
sudo nginx -t && sudo systemctl reload nginx
curl -I https://waha.kortex.com.br/api/health    # 401 esperado sem header
```
...
```bash
sudo tee /opt/kortex-waha/backup.env > /dev/null <<EOF
B2_ACCOUNT_ID=<key id>
B2_ACCOUNT_KEY=<key>
RESTIC_REPOSITORY=b2:kortex-waha-backup:/waha
RESTIC_PASSWORD=<senha forte armazenada no 1Password>
EOF
sudo chmod 600 /opt/kortex-waha/backup.env

source /opt/kortex-waha/backup.env
...
```cron
0 3 * * * kortex . /opt/kortex-waha/backup.env && restic backup /var/lib/docker/volumes/kortex-waha_waha-data --tag daily && restic forget --keep-daily 7 --keep-weekly 4 --keep-monthly 6 --prune
```
...
# em VPS de teste:
docker compose down
sudo rm -rf /var/lib/docker/volumes/kortex-waha_waha-data/*
. /opt/kortex-waha/backup.env
...
UptimeRobot (free tier, 5min) apontando pra `https://waha.kortex.com.br/ping` (rota pública sem auth). Alerta → email + WhatsApp via webhook do próprio app.
...
if curl -s -f http://127.0.0.1:3000/ping > /dev/null; then
  ...
  if [ "$FAILS" -ge 3 ]; then
    cd /opt/kortex-waha && docker compose restart waha
...
echo "*/1 * * * * kortex /usr/local/bin/waha-watchdog.sh" | sudo tee /etc/cron.d/waha-watchdog
```

### 8.3 Log shipping (opcional Fase 2)

Better Stack ou Datadog Agent → `docker logs kortex-waha`. Sem isso, logs ficam só locais e somem em rotação.
...
```
WAHA_API_BASE_URL=https://waha.kortex.com.br
