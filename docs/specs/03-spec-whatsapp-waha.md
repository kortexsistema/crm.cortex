...
A `WAHA_API_KEY` do servidor é o **hash SHA512 hex (lowercase) do plaintext**. O backend Kortex guarda **só** o plaintext em Vercel Encrypted Env Var; nunca a hash duplicada. Geração:
...
WAHA_DASHBOARD_USERNAME=admin_kortex
WAHA_DASHBOARD_PASSWORD=<senha forte gerada>
WAHA_S3_REGION=auto
WAHA_S3_BUCKET=kortex-waha-media
...
WAHA_BASE_URL=https://waha.kortex.internal
WAHA_WEBHOOK_PUBLIC_BASE_URL=https://api.kortex.com
...
server {
  ...
  server_name waha.kortex.internal;

  ssl_certificate     /etc/letsencrypt/live/waha.kortex.internal/fullchain.pem;
  ssl_certificate_key /etc/letsencrypt/live/waha.kortex.internal/privkey.pem;
...
1. Verificar logs WAHA (`docker logs kortex-waha --tail 500`).
...
- Domínio gerado pela Railway (`waha-kortex.up.railway.app`); usado em `WAHA_BASE_URL` (Vercel).
- Custo: $5-10/mês.
...
- Instalação:
  ...
  certbot --nginx -d waha.kortex.internal
  cd /opt/kortex-waha && docker compose up -d
  ```
