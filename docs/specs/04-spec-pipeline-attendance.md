...
- **ESLint** com `eslint-plugin-react-hooks`, `@typescript-eslint`, e plugin custom `eslint-plugin-kortex` (item §11.3).
...
Plugin `eslint-plugin-kortex` com regra `no-hardcoded-vocabulary`. Bloqueia em `.tsx` strings literais que parecem vocabulary:

```js
// eslint-plugin-kortex/rules/no-hardcoded-vocabulary.js
const FORBIDDEN = [
  "Cliente", "Clientes",
  "Pedido", "Pedidos",
  "Negócio", "Negócios",
...
Whitelist via `// kortex-allow-vocab: <razão>`. CI roda `eslint --max-warnings 0` no diff dos PRs.
