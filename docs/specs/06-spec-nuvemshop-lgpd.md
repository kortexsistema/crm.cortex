...
|---|---|---|
| App embedded vs External | **External** | Mais flexibilidade de UI custom (admin Kortex tem UX própria); evita iframe sandbox; consent renderizado no domínio Nuvemshop é suficiente |
...
export class NuvemshopAdapter implements EcommercePlatformAdapter {
  ...
  private async request<T>(ctx: AdapterContext, path: string, init: RequestInit & { idempotencyKey?: string } = {}): Promise<{ data: T; rateLimit: FetchPage<T>['rateLimit'] }> {
    ...
    const headers: Record<string, string> = {
      ...
      'User-Agent': 'Kortex/1.0 (rafael@maudibrasil.com.br)',
...
- UI custom no admin Kortex (sem iframe Nuvemshop sandbox)
- Permite callback em domínio próprio com cookie de sessão Kortex
...
## 8. Mapping Nuvemshop → Kortex

### 8.1 `customer.email/phone/identification` → `contacts`

| Campo Nuvemshop | Campo Kortex | Notas |
|---|---|---|
