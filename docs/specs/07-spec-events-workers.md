...
> Esta spec define o **bus interno** do Kortex: como módulos publicam eventos no banco, como workers consomem, quais crons rodam no Vercel, como tratamos retries, dead-letter, idempotência e observabilidade. É **transversal** — todos os outros sub-PRDs (02–06) emitem ou consomem deste bus.
...
- **Faz:** POST com HMAC SHA-256 signature, header `X-Kortex-Signature`, timeout 10s. Sucesso = 2xx.
...
- **Idempotência:** header `X-Kortex-Idempotency-Key = event.id`.
...
- **HTTP outbound (webhooks):** header `X-Kortex-Idempotency-Key = event_id`.
