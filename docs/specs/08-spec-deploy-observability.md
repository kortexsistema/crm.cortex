...
|---|---|---|---|---|---|
| Production | `main` | `app.kortex.com.br` + `admin.kortex.com.br` | projeto Pro `kortex-prod` | `waha.kortex.com.br` (Hostgator) | `production` |
| Staging | `staging` | `staging.kortex.com.br` | projeto Free `kortex-staging` | `waha-staging.kortex.com.br` (Hostgator mesmo VPS, container separado) | `staging` |
| Preview | qualquer PR | `*.vercel.app` | projeto Free `kortex-preview` (compartilhado) | mock/staging | `preview` |
...
- **Project:** `kortex-app` (Next.js) + `kortex-mcp` (MCP server fase 2).
...
```bash
git clone git@github.com:kortex/kortex.git
cd kortex
...
Migrations ficam em `supabase/migrations/YYYYMMDDHHMMSS_*.sql`. Seed em `supabase/seed.sql` cria 1 organization, 1 user `dev@kortex.local`, 1 pipeline default.
...
services:
  waha:
    ...
    container_name: kortex-waha-dev
...
ngrok http 3000 --domain=kortex-dev.ngrok.dev   # plano pago dá domínio fixo
```
...
```toml
[auth]
site_url = "https://app.kortex.com.br"
additional_redirect_urls = [
  "https://app.kortex.com.br/auth/callback",
  "https://admin.kortex.com.br/auth/callback",
]
...
- **Preview:** vars setadas em scope `Preview` apontando pra projeto Supabase `kortex-preview` compartilhado, WAHA mock ou staging, AI Gateway com `MOCK_AI=true` por default.
...
`/srv/kortex/docker-compose.yml`:
```yaml
services:
  waha:
    image: devlikeapro/waha-plus:latest
    container_name: kortex-waha
    restart: always
    networks:
      - waha-net
    expose:
    ...
    environment:
      ...
      WHATSAPP_DEFAULT_ENGINE: NOWEB
      WHATSAPP_HOOK_URL: https://app.kortex.com.br/api/webhooks/waha
...
Backup script `/srv/kortex/scripts/backup-sessions.sh`:
```bash
#!/usr/bin/env bash
set -euo pipefail
...
docker compose -f /srv/kortex/docker-compose.yml stop waha
tar czf "$BACKUP_DIR/sessions-$TS.tar.gz" -C /srv/waha sessions
docker compose -f /srv/kortex/docker-compose.yml start waha
# Upload pro Wasabi/R2
aws s3 cp "$BACKUP_DIR/sessions-$TS.tar.gz" \
  "s3://kortex-backups/waha-sessions/sessions-$TS.tar.gz" \
...
server {
    listen 80;
    server_name waha.kortex.com.br;
...
server {
    ...
    ssl_certificate     /etc/letsencrypt/live/waha.kortex.com.br/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/waha.kortex.com.br/privkey.pem;
...
docker run --rm -v /etc/letsencrypt:/etc/letsencrypt -v /var/www/certbot:/var/www/certbot \
  ...
  -d waha.kortex.com.br --email ops@kortex.com.br --agree-tos --no-eff-email
```
...
- 1 deploy por commit. URL `kortex-app-git-{branch}-{team}.vercel.app`.
...
```json
{
  ...
  "service":"kortex-app","env":"production","commit":"a1b2c3d",
...
|---|---|---|---|---|---|
| A1 | Sentry novo issue (unhandled exception em prod) | 1ª ocorrência | Slack #kortex-alerts | warn | dev on-call |
| A2 | Sentry issue volume spike | >50 events/h | Slack + PagerDuty | crit | dev on-call |
| A3 | WAHA session FAILED | qualquer | Slack #kortex-ops + email tenant | crit | DevOps |
| A4 | WAHA session STARTING >5min | sustentado | Slack #kortex-ops | warn | DevOps |
| A5 | LGPD `data_request` SLA D+5 | cron diário | Slack #kortex-lgpd + email DPO | warn | LGPD owner |
| A6 | LGPD `data_request` SLA D+6 | cron diário | PagerDuty | crit | LGPD owner |
| A7 | Rate limit Nuvemshop estourado | >5 429s/min | Slack #kortex-ops | warn | dev on-call |
...
> Convenção: cada runbook tem **Sintoma → Diagnóstico → Ação → Verificação → Pós-mortem**. Salvos também em `/srv/kortex/runbooks/` no VPS pra acesso offline.
...
**Diagnóstico.**
1. Confirmar via WAHA dashboard interno (port-forward ssh) ou `docker exec kortex-waha curl localhost:3000/api/sessions`.
...
2. **Comunicação:** email ao admin do tenant + Slack #kortex-ops com root-cause hypothesis.
...
**Diagnóstico.**
```bash
ssh ops@waha.kortex.com.br
docker logs kortex-waha --tail 200 | grep -i "session-{id}"
docker exec kortex-waha ls /app/.sessions/{sessionId}
```
...
- **WAHA banimento generalizado da conta Kortex (modelo BPO):** plano de migração pra API oficial Meta documentado no PRD-03 (Fase 2.5).
