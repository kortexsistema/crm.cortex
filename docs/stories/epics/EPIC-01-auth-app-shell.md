...
- [ ] Logout limpa cookie `sb-kortex-auth` + redireciona pra `/login`
...
| `cookie.sb-kortex-auth` | cookie | S-01.01 | Nome canônico do cookie de sessão Supabase em todos os ambientes |
...
5. Forçar nome de cookie: `cookieOptions: { name: 'sb-kortex-auth', sameSite: 'strict', httpOnly: true, secure: true }` (dev pode `secure: false` via env)
...
| t5 | rls | Cookie name canônico | Inspect Set-Cookie em login → `sb-kortex-auth=...` |
...
- **D-01.01**: Cookie de sessão chama-se `sb-kortex-auth` em todos os ambientes (locked)
...
When submete /login
Then cookie sb-kortex-auth é setado
...
Then sb-kortex-auth elevated, redirect /app/inbox
...
Server Action `signOut` que chama `supabase.auth.signOut()` server-side, deleta cookies (`sb-kortex-auth`, `active_org`, `mfa_pending` se existir, `sidebar_collapsed`), audit `auth.logout`, `redirect('/login')`. Botão dispara via `<UserMenu>` (componente vem em S-01.10, mas action precisa estar pronta).
...
When chama signOut
Then cookie sb-kortex-auth removido
...
|---|---|---|
| Cookie name divergente entre dev e prod quebra refresh silenciosamente | Alta | D-01.01: nome único `sb-kortex-auth` em todos envs; teste cobre |
...
- **ADR-14**: Cookie de sessão Supabase nomeado `sb-kortex-auth` em todos os ambientes (D-01.01)
...
| Wave | Story | Commit |
|------|-------|--------|
| 1 | S-01.01 Middleware proteção (cookie sb-kortex-auth) | `1310d4e` |
...
### Seed admin user
- Email: `rafael@maudibrasil.com.br` / Senha: `KortexAdmin@2026` (TROCAR APÓS PRIMEIRO LOGIN)
