...
**Goal:** Tornar o Kortex rodável 100% via `docker compose up` num VPS HostGator, sem quebrar o deploy Vercel atual.
...
docker build --build-arg NEXT_PUBLIC_SUPABASE_URL=https://x.supabase.co \
  ...
  -t kortex-app:test .
```
Expected: imagem builda. `docker run --rm -e SUPABASE_SERVICE_ROLE_KEY=... kortex-app:test` sobe e loga listen em `:3000` (pode falhar em deps externas, mas o processo inicia).
