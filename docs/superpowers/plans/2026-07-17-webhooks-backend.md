...
/** HMAC SHA-256 hex do raw body. Header: X-Kortex-Signature. */
export function verifyInboundSignature(rawBody: string, header: string | null, secret: string): boolean {
...
// 4. Fonte com secret: sem header X-Kortex-Signature → 401 e NENHUM lead;
...
export async function POST(req: NextRequest, ctx: RouteCtx): Promise<NextResponse> {
  ...
  const sigHeader = req.headers.get("x-kortex-signature");
...
//    com envelope {event, occurred_at, data}; header X-Kortex-Event correto;
//    SEM X-Kortex-Signature (config sem secret); body SEM organization_id.
// 2. Com secret: header X-Kortex-Signature presente e igual ao
...
export async function executeCallWebhook(
  ...
  const headers: Record<string, string> = {
    ...
    "X-Kortex-Event": ctx.event.event_type,
  };
  const secret = typeof config.secret === "string" ? config.secret : null;
  if (secret) {
    headers["X-Kortex-Signature"] = createHmac("sha256", secret).update(body).digest("hex");
  }
