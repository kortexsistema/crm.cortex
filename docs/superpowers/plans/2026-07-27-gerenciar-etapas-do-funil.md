...
- [ ] **Step 1: Mapa vivo** — `docs/architecture/agent-turn.workflow.json` (o rastreado; **não toque** em `kortex-system.*` nem `flywheel-proposal.*`, que são untracked de outra sessão). A tela entra com ao menos 2 arestas, e uma tem que ser `tela de etapas → mapa do funil` — as duas se completam. Rode o validador e re-renderize o HTML.
