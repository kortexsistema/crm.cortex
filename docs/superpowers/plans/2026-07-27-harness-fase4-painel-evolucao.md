...
Suba o dev server (`lsof -tiTCP:3000`; se não houver, `npm run dev` em background e espere `/api/v1/health`). Logue como `e2e-admin@kortex.test` (senha em `.e2e-creds.json`; o MFA é TOTP — secret no scratchpad, código via `generateTotp` de `./tests/e2e/utils/totp`, seis caixas de um dígito).
