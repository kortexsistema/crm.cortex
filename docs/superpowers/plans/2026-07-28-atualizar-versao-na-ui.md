...
**Goal:** O dono de uma instalação self-host atualiza o Kortex clicando num botão na própria tela, sem abrir terminal nem SSH.
...
- **Worktree:** `~/Kortex-update`, branch `feat/atualizar-pela-ui` (já criada a partir de `origin/main`). Não trabalhe no clone principal — ele está sujo com trabalho de outra sessão.
...
Primeira versão marcada do Kortex.
`;
...
const COMANDO_MANUAL = "cd Kortex && bash hostgator-setup-kit/update.sh";
...
set +e
KORTEX_AGENT_REPORT=1 \
KORTEX_AGENT_PREV_IMAGE="$PREV_IMAGE" \
KORTEX_AGENT_REPORT_CMD="$(declare -f report); report" \
...
export APP_IMAGE="ghcr.io/kortexsistema/kortex-v1:${TARGET_TAG#v}"
...
```bash
[ -n "${KORTEX_AGENT_REPORT:-}" ] && eval "${KORTEX_AGENT_REPORT_CMD}" backup   # após o backup
[ -n "${KORTEX_AGENT_REPORT:-}" ] && eval "${KORTEX_AGENT_REPORT_CMD}" codigo   # após o checkout
[ -n "${KORTEX_AGENT_REPORT:-}" ] && eval "${KORTEX_AGENT_REPORT_CMD}" banco    # após o baseline
```
