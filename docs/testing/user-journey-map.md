...
> Fonte da verdade do QA de produto do Kortex open-source. Cada caso aqui é
...
| M3 | README self-host aponta repo/imagem `kortex/*`; kit usa `melgarafael/*` | explorer webhooks | Alta |
...
imagem `ghcr.io/kortexsistema/kortex-v1:latest` — a mesma que o comprador recebe.
...
- O autenticador registra o nome fixo "Kortex", ignorando o `APP_NAME` que o
...
`KORTEX_GOV_INVARIANTS_EDIT=1` **declarando o uso no commit** (`685d6e7`) em vez
