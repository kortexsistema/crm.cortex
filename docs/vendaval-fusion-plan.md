# Fusão Vendaval → Kortex — briefing de execução (Terminal B)
...
**Um repositório só: o Kortex.** Fusão total do back-end do Vendaval (bom, mas nunca ligado à
realidade) dentro do Kortex (Next.js + TS + Supabase + WAHA + AI SDK — CRM real com chat ao vivo,
...
Kortex como borda — o contrato exato está em `~/vendaval/docs/specs/edge-contract.md` (referencia
arquivos e linhas reais do Kortex). Como agora é UM repo, a maquinaria de "dois sistemas" colapsa em
chamadas internas.

**Encaixes verificados no código real do Kortex:**
...
- Entrada: WAHA → `event_log` (RPC `emit_event`). O Kortex tem um dispatcher nativo (`dispatchAgents`).
...
- Modelo de dados do Kortex é o CANÔNICO: `organizations`/`contacts`/`conversations`/`messages`/
...
→ resposta aparece no CHAT do Kortex. *Prova:* você manda um WhatsApp de verdade a um número conectado e VÊ o
...
**Fase 2 — continuidade + config real.** Memória/RAG/follow-up no turno (usando histórico real do Kortex) +
...
- Do Kortex: siga o `CLAUDE.md`/`AGENTS.md`/MANIFEST do repo (migrations, RLS, LGPD nativa, multi-tenant).
