...
- **(A) RECOMENDADA — projeto Supabase NOVO dedicado** ("kortex-fusion-vps",
...
POST https://api.supabase.com/v1/projects
  {"name":"kortex-fusion-vps","organization_id":"pzaxvazodcffkfkpyqtl",
...
# usuário via Auth Admin API (email kortex-vps-admin@..., senha gerada) +
...
ssh ... 'mkdir -p /opt/kortex-fusion && git clone -b vendaval-fusion /opt/fusion.bundle /opt/kortex-fusion/app'
...
scp -P 22022 -i ~/.ssh/... /tmp/fusion-vps.env root@...:/opt/kortex-fusion/app/.env
ssh ... 'chmod 600 /opt/kortex-fusion/app/.env && rm -f /opt/fusion.bundle'
...
#     AGENT_DISPATCH_CONSUMER=engine · WAHA key + sha512 · APP_IMAGE=kortex-app:vps
...
# rede/volumes: os default do project-name kortex-fusion (nada da easypanel)

# B4. build local na VPS (~10-20 min; 4 cores é ok)
ssh ... 'cd /opt/kortex-fusion/app && docker compose -p kortex-fusion \
...
ssh ... 'docker compose -p kortex-fusion ... ps'  # tudo healthy; ss -tlnp → só as novas portas
```
...
no firewall; rollback = `docker compose -p kortex-fusion down` (só a nossa stack).
