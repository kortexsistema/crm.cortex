...
/**
 * Forçar Avanço de um enrollment VIVO (pular o wait_until, grace_timeout_ms, ou handoff_paused).
 * Define o next_eval_at para now() e acorda o motor imediatamente.
 */
export function useAdvanceFollowupEnrollment() {
  const qc = useQueryClient();
  return useMutation({
    mutationKey: ["followup", "enrollments", "advance"],
    mutationFn: async (enrollmentId: string) => {
      const res = await apiClient.post<{ data: { id: string; status: string } }>(
        `/api/v1/ai/followups/enrollments/${enrollmentId}/advance`,
        {},
      );
      return res.data;
    },
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ["followup", "queue"] });
      toast.success("O contato foi forçado a avançar.");
    },
    onError: (err) => {
      showApiError(err);
    },
