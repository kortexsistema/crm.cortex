...
# `/root/_kortex` faz o kit calcular `_kortex` enquanto os contêineres
# carregam `kortex`: a instalação deixa de se reconhecer e passa a se tratar
# como intrusa. Medido contra o docker compose v2.38.2 em `_kortex`,
# `-kortex`, `_-_crm` e `_123` — todos divergiam.
...
enter_project() {
  if [ -f "$COMPOSE" ]; then :;
  elif [ -f "kortex/$COMPOSE" ]; then cd kortex;
...
cron_tag() { printf '# kortex:%s:%s' "${PROJECT_DIR:-$PWD}" "${1:?papel da linha (drain|agent)}"; }
...
setup_update_agent_cron() {
  ...
  # a linha só funciona por acidente (instalação padrão em /root/kortex) e
