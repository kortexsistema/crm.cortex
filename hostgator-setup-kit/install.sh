#!/usr/bin/env bash
#
# Kortex — instalador self-host para VPS (HostGator).
#
...
REPO_URL="${REPO_URL:-https://github.com/kortexsistema/kortex-v1.git}"
...
# Caddy — inclusive o de outro Kortex instalado na mesma VPS — passava
...
e, se for mesmo um Traefik, ponha REVERSE_PROXY=traefik no .env e rode de novo."
  ...
  *)
    ...
    printf '\n%s\n'   "  1. Já é outro Kortex neste servidor? Então use aquele — entre na"
...
if [ -z "${NEXT_PUBLIC_SUPABASE_URL:-}" ] && [ -n "${SUPABASE_ACCESS_TOKEN:-}" ]; then
  ...
  _sb_out="$(bash "$KIT_DIR/supabase-provision.sh" "${APP_NAME:-Kortex}" "${SUPABASE_REGION:-sa-east-1}")" \
...
FIELDS=(
  ...
  "APP_NAME|Nome que aparece na interface (Enter para o padrão)|Kortex|||"
)
