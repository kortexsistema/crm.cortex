...
* * * * * curl -fsS -H "Authorization: Bearer SEGREDO" "https://crm.kortex.com.br/api/v1/cron/event-log-drain" >/dev/null 2>&1
*/5 * * * * cd /root/Aula-Youtube/Kortex && bash hostgator-setup-kit/agent.sh >/dev/null 2>&1'
...
NOVO_TAG='# kortex:/root/instalacao-nova'
NOVA_URL='https://crm-novo.exemplo.com.br/api/v1/cron/event-log-drain'
cron_ok "o drain do vizinho sobrevive"  'crm.kortex.com.br/api/v1/cron/event-log-drain' \
        "$NOVO_TAG" "$NOVA_URL" "* * * * * curl \"$NOVA_URL\" $NOVO_TAG"
cron_ok "o agente do vizinho sobrevive" 'cd /root/Aula-Youtube/Kortex && bash hostgator-setup-kit/agent.sh' \
...
reexec="$(printf '%s\n' "$CRONTAB_VIZINHO" | cron_merge '# kortex:/root/Aula-Youtube/Kortex' \
          'cd /root/Aula-Youtube/Kortex && bash hostgator-setup-kit/agent.sh' \
          '*/5 * * * * cd /root/Aula-Youtube/Kortex && bash hostgator-setup-kit/agent.sh # kortex:/root/Aula-Youtube/Kortex')"
...
TAG_DRAIN="# kortex:${DIR}:drain"; TAG_AGENT="# kortex:${DIR}:agent"
...
# Caddy de OUTRO Kortex na mesma VPS — caía no ramo "portas livres", e a
...
dono_ok "Caddy de outro Kortex é encontrado" \
...
dec_ok "Caddy de OUTRO Kortex → bloqueia" bloqueia "80 e 443" "outro"     "crm" "caddy:2-alpine" "outro-caddy-1"
...
# O compose faz TrimLeft("_-") no basename. Sem isso, uma pasta /root/_kortex
# faz o kit calcular "_kortex" enquanto os contêineres carregam "kortex" — a
...
np_ok /root/kortex  kortex
np_ok /root/Kortex  kortex
np_ok /root/_kortex    kortex
np_ok /root/-kortex    kortex
np_ok /root/_-_crm       crm
np_ok /root/_123         123
np_ok /root/kortex.crm kortex
...
if [ ! -s "$CRONTAB_SANDBOX" ] || ! grep -q '# kortex:' "$CRONTAB_SANDBOX"; then
