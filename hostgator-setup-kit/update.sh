#!/usr/bin/env bash
# Atualiza o Kortex na VPS: código novo + banco + app — com BACKUP antes e
...
image_desatualizada() {
  local img="${APP_IMAGE:-ghcr.io/kortexsistema/kortex-v1:latest}" local_d remote_d
...
[ -n "${KORTEX_AGENT_REPORT:-}" ] && eval "${KORTEX_AGENT_REPORT_CMD}" backup
...
[ -n "${KORTEX_AGENT_REPORT:-}" ] && eval "${KORTEX_AGENT_REPORT_CMD}" codigo
...
[ -n "${KORTEX_AGENT_REPORT:-}" ] && eval "${KORTEX_AGENT_REPORT_CMD}" banco
