# agent-engine — notas do porte Vendaval → Kortex (fusão)
...
| Vendaval (antigo) | Kortex (canônico) |
|---|---|
...
- AI SDK do repo: `ai` ^6 + `@ai-sdk/anthropic` ^3 (NÃO v7/v4 do Vendaval). Espelhar o uso de
  `generateText`/usage que `lib/ai/runtime/agent.ts` do Kortex já faz.
...
- Canvas/console do Vendaval: descartados (UI do Kortex substitui).
