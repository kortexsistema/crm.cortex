...
import { MetaChannelAdapter } from '../edge/channel/meta-adapter';
  ...
  return async (job: JobRow, pool: pg.Pool, ctx: { workerId: string }): Promise<void> => {
    ...
    if (payload.followup_enrollment_id !== undefined) {
      await runFlowDrivenTurn(deps, job, pool, ctx, clock, target, {
        ...
        templateVariables: payload.template_variables,
      });
      return;
    }
...
async function runFlowDrivenTurn(
  ...
  if (input.purpose === 'send_message') {
    if (input.templateId !== undefined) {
      // Envio DETERMINÍSTICO de Template Oficial (sem LLM).
      if (await isLeadInHandoff(pool, target.tenantId, target.leadId)) {
        runLog.info('re-entrada por template pulada — lead silenciado (handoff/opt-out)', { kind: job.kind });
        await complete(pool, { organizationId: target.tenantId, enrollmentId, nodeId, result: { kind: 'sent' } });
        return;
      }

      const engineEnv = loadEnv();
      const channel = (deps.channel ?? ((p: pg.Pool) => 
        engineEnv.MESSAGE_PROVIDER === 'META_API' 
          ? new MetaChannelAdapter(p, deps.crmCfg) 
          : new WahaChannelAdapter(p, deps.crmCfg)
      ))(pool);
      ...
      const chain = await runBeforeSend({
        ...
        body: 'TEMPLATE_PLACEHOLDER', // passamos um placeholder pois o Meta renderizará na API
        ...
        send: async (b) => {
          return await channel.send({
            ...
            template: {
              ...
              language: 'pt_BR',
              values: input.templateVariables ?? {},
            },
          });
        },
      });

      if (chain.status !== 'sent') {
        throw new Error(`envio de template vetado pelo guardrail before_send: ${chain.status}`);
      }

      const res = chain.outcome;
      if (res.kind === 'failed' || res.kind === 'unavailable') {
        throw new Error(`erro ao enviar template via channel adapter: ${res.kind}`);
      }
