...
/**
 * SaaS Platform Markup.
 * Multiplica o custo-base para compor a margem de lucro (ex: 2.0 = 100% de margem).
 */
export const SAAS_MARKUP_MULTIPLIER = 2.0;
...
/**
 * Custo final tarifado ao tenant, aplicando a margem de lucro.
 * Arredondado para cima para garantir que centavos fracionários não sejam perdidos.
 */
export function billedCostCents(model: string, usage: TokenUsage): number {
  const baseCost = costCents(model, usage);
  if (baseCost === null) return 0;
  return Math.ceil(baseCost * SAAS_MARKUP_MULTIPLIER);
}
