...
import { costCents, billedCostCents, SAAS_MARKUP_MULTIPLIER } from './pricing';
...
export async function runModelCall(db: pg.Pool, cfg: LlmEdgeConfig, input: RunModelCallInput, deps: RunModelCallDeps = {}) {
  ...
  // FIRE AND FORGET: Deduct from Ledger (Fase 2.1)
  const billedAmount = billedCostCents(model, usage);
  if (billedAmount > 0) {
    db.query(
      `insert into tenant_billing_ledgers (organization_id, transaction_type, amount_cents, description, metadata)
       values ($1, 'llm_usage', $2, $3, $4)`,
      [
        input.tenantId,
        -billedAmount, // Negative to deduct
        `Consumo de IA (${model})`,
        JSON.stringify({
          provider: config.provider,
          model,
          ...
          markup_multiplier: SAAS_MARKUP_MULTIPLIER
        })
      ]
    ).catch(err => {
      deps.log?.error('llm: falha ao debitar do ledger', { error: String(err) });
    });
  }
