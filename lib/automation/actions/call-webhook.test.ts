...
describe("executeCallWebhook", () => {
  ...
  it("sucesso: envia envelope correto, sem assinatura, sem organization_id", async () => {
    ...
    expect(received!.headers["x-kortex-event"]).toBe("lead.created");
    expect(received!.headers["x-kortex-signature"]).toBeUndefined();
  ...
  it("com secret: header de assinatura HMAC-sha256 do body", async () => {
    ...
    expect(received!.headers["x-kortex-signature"]).toBe(expectedSig);
