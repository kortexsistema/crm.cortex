...
function condBranchOption(value: string, label: string): EdgeConditionOption {
  return {
    key: `cond_branch:${value}`,
    label,
    condition: { type: "cond_branch", value },
  };
}
...
export function edgeConditionOptions(sourceNode: FlowNode | undefined): EdgeConditionOption[] {
  ...
  if (sourceNode?.type === "condition") {
    if ("rules" in sourceNode.config) {
      return [
        ALWAYS_OPTION,
        ...sourceNode.config.rules.map((r) => condBranchOption(r.id, r.label)),
      ];
    }
...
export function conditionKey(condition: FlowEdge["condition"]): string {
  switch (condition.type) {
    ...
    case "cond_branch":
      return `cond_branch:${condition.value}`;
  }
}
