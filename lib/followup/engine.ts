...
export interface FollowupJobRequest {
  ...
  payload: {
    ...
    guidance?: string;
    /** action (mode 'template') — Task 5.1: identificador e variáveis para api de envios determinísticos */
    template_id?: string;
  ...
  if (node.type === "action") {
    ...
    if (node.config.mode === "template") {
      return { 
        ...
        ...(node.config.template_variables ? { template_variables: node.config.template_variables } : {})
      };
    }
  }
