...
export const waitConfigSchema = z
  .discriminatedUnion('mode', [
    ...
    z.strictObject({
      mode: z.literal('event_relative'),
      offset_ms: z.number().int().min(-7_776_000_000).max(7_776_000_000),
    }),
  ])
 ...
 */
const baseConditionCheckSchema = z.strictObject({
  field: z.enum([
    'lead_stage',
    'tag',
    'steps_taken',
...
const legacyConditionConfigSchema = z.strictObject({
  combinator: z.enum(['and', 'or']).default('and'),
  checks: z.array(baseConditionCheckSchema).min(1).max(10),
});

const switchConditionConfigSchema = z.strictObject({
  rules: z.array(
    z.strictObject({
      id: z.string().min(1),
  ...
  fallback_label: z.string().max(60).optional(),
});

/**
 * Condition node configuration.
 * Evaluates multiple checks against lead state using boolean logic.
 * Supports legacy single-rule mode or the new multi-rule switch mode.
 */
export const conditionConfigSchema = z.union([switchConditionConfigSchema, legacyConditionConfigSchema]);
...
export const flowEdgeSchema = z.strictObject({
  ...
  condition: z.discriminatedUnion('type', [
    ...
    z.strictObject({
      type: z.literal('cond_branch'),
      value: z.string(), // ID da regra ou 'fallback'
    }),
  ]),
});
