...
export function selectEdge(edges: FlowEdge[], from: string, match: EdgeMatch): FlowEdge | null {
  ...
  const exact = candidates.find((e) => {
    ...
    if (match.type === "cond_branch") return e.condition.type === "cond_branch" && e.condition.value === match.value;
  ...
  switch (node.type) {
    ...
    case "wait": {
      if (!waitElapsed) {
        ...
        if (node.config.mode === "fixed") durationMs = node.config.duration_ms;
        else if (node.config.mode === "event_relative") {
          // TODO: implement real event payload extraction. Using offset_ms directly for now as a fallback.
          durationMs = node.config.offset_ms > 0 ? node.config.offset_ms : 0; 
        } else {
          durationMs = node.config.max_ms; // smart mode uses max_ms clamped
        }
    ...
    case "condition": {
      ...
      const edgeMatch: EdgeMatch = typeof result === "boolean" 
        ? { type: "cond_result", value: result } 
        : { type: "cond_branch", value: result };
        
      const edge = selectEdge(edges, node.id, edgeMatch);
