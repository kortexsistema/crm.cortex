...
// ---- reação 4: case.resolved (fechado/resolvido) -----------------------

async function reactToCaseResolved(
  db: ReactivityAdminClient,
  clock: () => Date,
  row: EventRow,
): Promise<ReactivitySummary> {
  // A payload do evento case.resolved deve conter o contact_id
  const contactId = strOrNull(row.payload.contact_id);
  if (!contactId) return { matched: false, reacted: 0 };

  const live = await db.loadLiveEnrollmentsForContact(row.organization_id, contactId);
  if (live.length === 0) return { matched: true, reacted: 0 };
  
  // Cancela todos os enrollments vivos do contato pois o caso foi resolvido
  const reacted = await cancelAll(
    db,
    row.organization_id,
    row.id,
    live,
    "handoff", // assumindo que a resolução humana ou de IA encerra o fluxo
    "case_resolved",
    "reactivity_case_resolved",
    clock
  );
  return { matched: true, reacted };
}

// ---- reação 5: message.failed (falha de entrega) -----------------------

async function reactToMessageFailed(
  db: ReactivityAdminClient,
  clock: () => Date,
  row: EventRow,
): Promise<ReactivitySummary> {
  ...
  // Aborta/Pausa enrollments correspondentes à falha de mensagem
  const reacted = await cancelAll(
    db,
    row.organization_id,
    row.id,
...
export async function applyReactivityEvent(
  ...
  switch (row.event_type) {
    ...
    case "case.resolved":
      return reactToCaseResolved(db, clock, row);
    case "message.failed":
      return reactToMessageFailed(db, clock, row);
    default:
