...
2. **Aprovar com ressalvas**: mesmo arquivo + escrever as ressalvas como respostas
   na inbox e/ou ajustar features da próxima fase (commit com `KORTEX_GOV_PLAN_EDIT=1`).
3. **Recusar**: NÃO criar o `.approved`. Em vez disso:
   ...
   `KORTEX_GOV_PLAN_EDIT=1`. O loop nunca reabre nem remove o `.rejected` por
...
KORTEX_GOV_PHASE_MERGE=1 git push origin main   # o ÚNICO push que o pre-push aceita
git branch gov/<FASE+1> main
```

**Opção B — Pull Request (o push é da branch da fase; o merge é humano no GitHub):**
```bash
KORTEX_GOV_PHASE_MERGE=1 git push origin gov/<FASE>
...
`KORTEX_GOV_PHASE_MERGE=1`, exportada só aqui) torna isso física, não instrução.
