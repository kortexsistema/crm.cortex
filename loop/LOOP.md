# LOOP.md — prompt canônico da sessão do gov-loop — Kortex (Governança de Atendimento)

> Você é UMA sessão DESCARTÁVEL do loop de construção do épico de governança de
> atendimento do Kortex (spec-mãe: `docs/specs/13-spec-governanca-atendimento.md`;
...
2. Existe `loop/checkpoints/G<N>-report.md` SEM o par `loop/checkpoints/G<N>.approved`
   ...
   `KORTEX_GOV_PLAN_EDIT=1` — o pre-commit barra sem ela). Sem item `answered`
   sobre a recusa → saia.
...
> (`loop/loop.config.json → main_checkout`, hoje `/Users/rafaelmelgaco/Kortex`).
...
- **Briefing ao subagente** (padrão dispatch — contexto completo, uma tarefa):
  ...
  toca); e as **restrições de 1ª ordem do Kortex** (o CLAUDE.md do repo é a
...
1. **Commit atômico** na branch da fase — a sessão inteira é UM commit:
   ...
   (test.fails → teste normal), o commit exporta `KORTEX_GOV_INVARIANTS_EDIT=1`
...
2. **`plan/features.json`**: marque a feature via
   ...
   **NUNCA com Edit/Write direto** — o hook PreToolUse bloqueia, e o pre-commit
   ...
   Mudar o plano é ato humano, com `KORTEX_GOV_PLAN_EDIT=1` exportada.
...
- **Nunca `git push`.** Push só existe no ritual de virada de fase (CHECKPOINT.md).
  ...
  `KORTEX_GOV_PHASE_MERGE=1` — variável que só o ritual de virada exporta.
...
- **`tests/invariants/**` é o eval do épico — semi-congelado.** ADICIONAR arquivo
  ...
  `freeze-invariants.sh`) sem `KORTEX_GOV_INVARIANTS_EDIT=1`. A exceção legítima
...
- **Migration fora da tripla não commita.** `loop/hooks/check-migration-triple.sh`
  ...
  branches locais (bypass só orientado pelo dono: `KORTEX_GOV_MIGRATION_EDIT=1`).
