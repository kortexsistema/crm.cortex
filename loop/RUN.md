...
**Onde o loop RODA: no checkout principal — `/Users/rafaelmelgaco/Kortex`
(`loop/loop.config.json → main_checkout`) — DEPOIS que a branch `gov/setup` for
mergeada em `main`.** O worktree `Kortex-gov` existe só para montar a
...
- **Hooks determinísticos** (a parte do protocolo que não é prosa):
  ...
  `KORTEX_GOV_PLAN_EDIT=1`; tripla de migration com NNNN validado contra todas
  as branches locais; freeze de `tests/invariants/**`) e `pre-push` (push só com
  `KORTEX_GOV_PHASE_MERGE=1`, exportada só pelo ritual de virada).
...
- `plan/features.json` populado (26 features G1-01..G6-04); `plan/progress.md`
  ...
  ```json
  {
    ...
    "main_checkout": "/Users/rafaelmelgaco/Kortex",
    "parallel_ui_lane": false,
    "smoke": "pnpm typecheck && pnpm lint && pnpm test:unit"
  }
  ```
  ("hoje" do teto diário é no fuso America/Sao_Paulo.)
- Slash command (modo interativo): `.claude/commands/kortex-gov-loop.md`.
  **`/kortex-gov-loop`, não `/loop`** — `/loop` colide com a skill built-in
...
```bash
cd /Users/rafaelmelgaco/Kortex && claude
> /kortex-gov-loop
```
Uma sessão = uma feature. Quer outra feature, abra OUTRA sessão (`claude` de novo
ou `/clear` antes de `/kortex-gov-loop`). Nunca encadeie duas features na mesma
...
#!/usr/bin/env bash
set -u
REPO="/Users/rafaelmelgaco/Kortex"; cd "$REPO" || exit 1
...
LOCKDIR="/tmp/kortex-gov-loop.lock.d"
...
`~/Library/LaunchAgents/com.kortex.gov-loop.plist` com `ProgramArguments`
`/bin/bash -lc '/Users/rafaelmelgaco/Kortex/loop/run-session.sh'` e
...
1. **Nunca `git push` sem checkpoint aprovado — enforcement físico**: o hook
   `loop/hooks/pre-push` recusa qualquer push sem `KORTEX_GOV_PHASE_MERGE=1`, e
...
5. **Imutabilidade do plano — enforcement físico**: o pre-commit
   ...
   `KORTEX_GOV_PLAN_EDIT=1` (sessão humana). E a mutação legítima é SÓ via
...
6. **`tests/invariants/**` semi-congelado — enforcement físico**: PreToolUse
   ...
   bloqueia M/D sem `KORTEX_GOV_INVARIANTS_EDIT=1`. Adição (A) passa livre —
