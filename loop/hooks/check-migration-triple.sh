...
# Bypass (correção orientada pelo dono): KORTEX_GOV_MIGRATION_EDIT=1.
set -euo pipefail

[ "${KORTEX_GOV_MIGRATION_EDIT:-0}" = "1" ] && exit 0
...
if ! grep -qx 'supabase/baseline.sql' <<<"$staged"; then
  ...
  echo "Sem o baseline, self-hosters nunca recebem a mudança. Correção orientada pelo dono: KORTEX_GOV_MIGRATION_EDIT=1." >&2
  exit 1
fi
...
if ! grep -qx 'supabase/migrations/MANIFEST.md' <<<"$staged"; then
  ...
  echo "A tripla é indivisível (CLAUDE.md §Migrations): migrations/*.sql + baseline.sql + MANIFEST.md." >&2
  echo "Correção orientada pelo dono: KORTEX_GOV_MIGRATION_EDIT=1." >&2
  exit 1
fi
