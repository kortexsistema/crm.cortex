  ...
  Edit|Write)
    ...
    case "$path" in
      ...
      *tests/invariants/*)
        ...
        if [ -f "$path" ]; then
          deny "tests/invariants/** é congelado — invariante existente não se edita (flip legítimo de test.fails: sessão humana com KORTEX_GOV_INVARIANTS_EDIT=1 no commit)."
        fi
        ;;
    esac
    ;;
