...
# Adicionar/remover/redefinir features é ato humano: KORTEX_GOV_PLAN_EDIT=1.
set -euo pipefail

# Sessão humana / lane de features declarada explicitamente
[ "${KORTEX_GOV_PLAN_EDIT:-0}" = "1" ] && exit 0
...
if ! git cat-file -e HEAD:plan/features.json 2>/dev/null; then
  echo "pre-commit BLOQUEADO: criação de plan/features.json exige KORTEX_GOV_PLAN_EDIT=1 (sessão humana)." >&2
  exit 1
fi
...
if [ "$(jq -S 'del(.features)' <<<"$old")" != "$(jq -S 'del(.features)' <<<"$new")" ]; then
  ...
  echo "Mudar o plano é ato humano: KORTEX_GOV_PLAN_EDIT=1." >&2
  exit 1
fi
...
if [ "$(jq -S "$strip" <<<"$old")" != "$(jq -S "$strip" <<<"$new")" ]; then
  ...
  echo "Sessão humana/lane de features: re-execute o commit com KORTEX_GOV_PLAN_EDIT=1." >&2
  exit 1
fi
