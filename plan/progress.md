# Progress — gov-loop (Kortex · Governança de Atendimento)
...
- Próximo: dono revisa `gov/setup`, mergeia em `main`, e o loop abre G1
  (`/kortex-gov-loop`). G1-06 é human_input — as 5 decisões de produto.
...
- Handoff: próximas sessões (G2-02+) delegadas ao terminal Arquiteto e Executor
  (cooperação A2A retomada pelo dono); Maestro-Kortex vira watchdog.
...
- Flip da catraca: 2 it.fails GAP(G2) de gov-1-rbac viraram testes normais
  (única mudança no arquivo; commit com KORTEX_GOV_INVARIANTS_EDIT=1).
...
- INB-07 aprovado → feature G4-00 (hardening SECURITY DEFINER anon) criada no
  plano com KORTEX_GOV_PLAN_EDIT=1 (ca36202); agrupa INB-09.
...
- 17:11: Terminal B criou loop/STOP alegando git clean ~17h apagando trabalho
  ...
  no worktree ../Kortex-vendaval. Principal chegou a ser trocado pra
