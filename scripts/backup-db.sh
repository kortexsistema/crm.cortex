#!/usr/bin/env bash
# Backup do Postgres (Supabase) do Kortex — schema public completo
# (CRM + harness do agente). Roda no host ou num cron da VPS:
#   0 3 * * * /path/repo/scripts/backup-db.sh /var/backups/kortex
...
find "$DIR" -name 'kortex-*.dump' -mtime +"$RETENTION_DAYS" -delete
