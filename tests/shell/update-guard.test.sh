...
cat > "$PROJ/.env" <<ENV
APP_IMAGE=ghcr.io/kortexsistema/kortex-v1:latest
...
check ".env aponta para a imagem da versão instalada" grep -q '^APP_IMAGE=ghcr.io/kortexsistema/kortex-v1:1.1.0$' .env
...
check "a imagem do .env continua intacta" grep -q '^APP_IMAGE=ghcr.io/kortexsistema/kortex-v1:latest$' .env
...
check "NÃO reescreveu a imagem do .env" grep -q '^APP_IMAGE=ghcr.io/kortexsistema/kortex-v1:latest$' .env
