...
export async function runBudgetChecker(): Promise<BudgetCheckerStats> {
  ...
  if (creditsData) {
    for (const c of creditsData as CreditRow[]) {
      creditsMap.set(c.organization_id, c.balance_cents);
    }
  }
  ...
  for (const b of budgets) {
    ...
    // ---- Throttle / disable at 100% OR Zero Credits --------------------------------------
    const creditBalance = creditsMap.get(b.organization_id) ?? 0;
    const isOutofCredits = creditBalance <= 0 && creditsMap.has(b.organization_id);
    const shouldBlock = pct >= 100 || isOutofCredits;

    if (shouldBlock && !b.is_throttled && !b.is_disabled) {
      // If out of credits, it's ALWAYS a hard disable. If just budget hit, follow action_at_100pct.
      const action = isOutofCredits || b.action_at_100pct === "disable" ? "disable" : "throttle";
      const patch =
